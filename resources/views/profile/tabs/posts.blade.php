<div class="flex flex-col gap-3">
    ...
    <div class="grid grid-cols-12 gap-3 items-start" style="height: 800px;">
        ...
        <div class="col-span-8 bg-gray-900 rounded-3xl overflow-y-auto scroll-custom h-full" id="feed-scroll">
            ...
            @if ($user['id'] === Auth::id())
                <div class="flex gap-1 w-full bg-gray-900 rounded-xl shadow-lg p-4">
                    <a href="{{ route('profile.show', $user['id']) }}" class="w-10 h-10 mr-2 rounded-full overflow-hidden shrink-0 group-hover:scale-110 transition-transform">
                        <img src="{{ $user['avatar'] ? $user['avatar'] : asset('img/perfil.jpg') }}" alt="Avatar" class="w-full h-full object-cover">
                    </a>
                    ...
                    <button type="button"
                        ...
                        <span id="textSpanPost" data-placeholder="¿Qué estás pensando, {{ explode(' ', $user['fullname'])[0] }}?"  class="flex-1 text-left">
                            ¿Qué estás pensando, {{ explode(' ', $user['fullname'])[0] }}?
                        </span>
                    </button>

                    <div class="flex px-2 items-center gap-4 text-gray-400">
            ...
            <div class="flex flex-col gap-4 p-4">
                @if($posts->isNotEmpty())
                    @foreach ($posts as $post)
                        <div id="{{ $post['id'] }}" class="flex gap-3 w-full bg-gray-800/50 rounded-2xl p-4 cursor-pointer transition-all duration-200 border border-gray-700/50">
                            <div class="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-200">
                                <img src="{{ $post['user']['avatar'] ? $post['user']['avatar'] : asset('img/perfil.jpg') }}" alt="Avatar" class="w-full h-full object-cover">
                            </div>
                            <div class="flex flex-col gap-1 w-full min-w-0">
                                <div class="grid">
                                    <div class="flex justify-between">
                                        <div class="flex items-center gap-2">
                                            <span class="text-white font-bold text-sm">{{ $post['user']['fullname'] }}</span>
                                    ...
                                    <div class="grid">
                                        @if($post->contenido)
                                            <p class="text-gray-300 text-base leading-relaxed">{{ $post->contenido }}</p>
                                        @endif
                                        
                                        @if($post->multimedia->isNotEmpty())
                                            @php
                                                $mediaVisual = $post->multimedia->whereIn('tipo', ['multimedia', 'gif']);
                                                $files = $post->multimedia->where('tipo', 'file');
                                            @endphp

                                            {{-- Contenedor visual --}}
                                            @if($mediaVisual->isNotEmpty())
                                                <div class="dataMultimedia grid grid-cols-2 gap-3 mt-2">
                                                    @foreach($mediaVisual as $media)
                                                        @if(in_array($media->tipo, ['multimedia', 'gif']))
                                                            <img src="{{ $media->url }}" class="multimediaItem object-cover aspect-square rounded-xl overflow-hidden">
                                                        @else
                                                            <video src="{{ $media->url }}" controls class="multimediaItem object-cover aspect-square rounded-xl overflow-hidden"></video>
                                                        @endif
                                                    @endforeach
                                                </div>
                                            @endif
                                            ...
                                            @if($files->isNotEmpty())
                                                <div class="flex flex-col gap-2 mt-3">
                                                    @foreach($files as $media)
                                                        <a href="{{ $media->url }}" class="w-full bg-gray-700 hover:bg-gray-600 transition-all text-gray-100 rounded-md px-4 py-3 flex items-center justify-between gap-3">
                                                            <div class="flex gap-2 items-center">
                                                                <svg class="w-5 h-5 text-yellow-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                                            ...
                                                            <svg class="w-5 h-5 text-yellow-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                                                ...
                                                                <path stroke-linecap="round" stroke-linejoin="round" d="M4 17v1a2 2 0 002 2h12a2 2 0 002-2v-1" />
                                                            </svg>
                                                        </a>
                                                    @endforeach
                                                </div>
        ...
        <div class="col-span-4 bg-gray-900 rounded-3xl p-5 flex flex-col gap-4 h-full">
            ...
            @if($user['bio'])
                <p class="text-gray-400 text-sm leading-relaxed">{{ $user->bio }}</p>
            @else
            ...
            <div class="flex justify-between items-center">
                ...
                <span class="text-gray-500 text-xs">{{ $user->seguidores_count ?? 0 }} en total</span>
            </div>
                    ...
                    @foreach ($seguidores as $seguidor)
                        <div class="flex flex-col items-center gap-2 bg-gray-800 rounded-xl p-3 hover:bg-gray-700 transition-all duration-200 cursor-pointer">
                            ...
                            <div class="w-full text-center">
                                <p class="text-white text-xs font-bold leading-tight truncate">{{ $seguidor->fullname }}</p>
